...
generate_release_binaries:
	mkdir -p release; \
	GOOS=darwin GOARCH=amd64 ${GOBUILD} -o release/tag . && tar -C release -cvzf release/tag_darwin_amd64.tar.gz tag; \
	GOOS=darwin GOARCH=arm64 ${GOBUILD} -o release/tag . && tar -C release -cvzf release/tag_darwin_arm64.tar.gz tag; \
	...
	GOOS=linux GOARCH=arm64 ${GOBUILD} -o release/tag . && tar -C release -cvzf release/tag_linux_arm64.tar.gz tag; \
